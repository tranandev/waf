...
<table id="RuleTableurl" class="table table-striped table-bordered thead-dark" style="width:98%; margin-left: 20px;">
        <thead>
            <tr>
                ...
                <th>Description</th>
                <th>Group Rule</th>
                <th id="tbedit_u">Edit</th>
            </tr>
        </thead>
    </table>
<button class="btn btn-info" style="float:right; margin-top: 10px; margin-right:15px;" id="addUrl" onclick="AppendUrl()"><i class="fa fa-plus" style="padding-right:10px;"></i><span class="glyphicon glyphicon-plus-sign"></span> Add New</button>
</div>
...
$(document).ready(function() {
        table_url = $('#RuleTableurl').DataTable( {
        lengthChange: false,
        ajax: "{{route('get-url-dataTable')}}",
        columns: [
            { data: null, render: function ( data, type, row ) {
                return row.host + '/' + row.url;
            } },
            { data: "description" },
            { data: null, render: function ( data, type, row ) {
                return row.groupruleurl.name;
            } },
            { data: null, render: function ( data, type, row ) {
                return '<i style="margin-left:13px;" class="fa fa-lg fa-edit" title="Edit" aria-hidden="true" id="editUrl_'+ data.id +'" onclick="EditUrl(\''+ data.id +'\')"></i><i style="margin-left: 20px;" class="fa fa-lg fa-trash-o" title="Delete" aria-hidden="true" id="deleteUrl_'+ data.id +'" onclick="DeleteUrl(\''+ data.id +'\')"></i>';
            } }
        ],
        select: true,
        "initComplete": function(settings, json) {
            $("#tbedit_u").css('width', '60');
        }
    } );
});
...
function AppendUrl() {
    tr = $(".fa-check");
    if (!tr.length){
        $('#RuleTableurl').append('<tr id="URLRuleAppend" role="row" class="even"><td class="sorting_1"><input id="addUrlRule" style="width: 100%;" value=""></td><td><input id="addUrlRuleDescription" style="width: 100%;" value=""></td><td><select id="addGroupRule"> <option></option>' + '<?php foreach ($group_rule as $value) {echo "<option value=\"$value->id\">$value->name</option>";} ?>'+'</select></td><td><i style="margin-left:13px;" class="fa fa-lg fa-check" title="Save" aria-hidden="true" id="saveIpRule" onclick="SaveUrl()"></i><i style="margin-left: 20px;" class="fa fa-lg fa-close" title="Delete" aria-hidden="true" id="deleteUrl" onclick="reloadUrlRuleTable()"></i></td></tr>');    
    }
}

function reloadUrlRuleTable() {
    value = [];
    table_url.ajax.reload(null, false );
}
